            ...
            <ul role="menu" class="dropdown-menu pull-right">
              <li>
                <?php if(ltrim(rtrim($item->COD_ESTADO)) == ''): ?> 
                    <a href="<?php echo e(url('/detalle-comprobante-nota-credito-administrator/'.$procedencia.'/'.$idopcion.'/'.substr($item->COD_DOCUMENTO_CTBLE, 0,6).'/'.Hashids::encode(substr($item->COD_DOCUMENTO_CTBLE, -10)))); ?>">
                      Registro XML
                    </a>
                <?php else: ?>
                  ...
                  <?php else: ?>
                    <?php if($item->COD_ESTADO != 'ETM0000000000001'): ?>
                      <?php if($item->COD_ESTADO != 'ETM0000000000006'): ?>
                        <a href="<?php echo e(url('/detalle-comprobante-nota-credito-validado/'.$idopcion.'/'.substr($item->COD_DOCUMENTO_CTBLE, 0,6).'/'.Hashids::encode(substr($item->COD_DOCUMENTO_CTBLE, -10)))); ?>">
                          Detalle de Registro
                        </a>
                      <?php else: ?>
                        <a href="<?php echo e(url('/detalle-comprobante-nota-credito-administrator/'.$procedencia.'/'.$idopcion.'/'.substr($item->COD_DOCUMENTO_CTBLE, 0,6).'/'.Hashids::encode(substr($item->COD_DOCUMENTO_CTBLE, -10)))); ?>">
