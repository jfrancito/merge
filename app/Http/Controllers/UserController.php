...
class UserController extends Controller
{
	...
	public function actionUpdateSerieCorrelativoCpe()
	{
	...
	public function actionAjaxModalVerCuentaBancariaOC(Request $request)
	{
		...
		$bancos_sin_cuenta = CMPCategoria::where('TXT_GRUPO', '=', 'BANCOS_MERGE')
			->where('IND_ACTIVO', '=', 0)
			->get();
		...
		return View::make(
			...
			[
				...
				'cuentabancarias' => $cuentabancarias,
				'bancos_sin_cuenta' => $bancos_sin_cuenta,
				'idoc' => $idoc,
				'idopcion' => $idopcion,
				'ajax' => true,
			]
	...
	public function actionCambiarCuentaCorriente($empresa_id, $banco_id, $nro_cuenta, $moneda_id, $idoc, $idopcion, Request $request)
	{
		...
		if (count($cuentabancarias) > 0) {
			...
			//geolocalizacion
			$device_info = $request['device_info'];
			$this->con_datos_de_la_pc($device_info, $fe_documento, 'CAMBIO DE CUENTA BANCARIA ' . $fe_documento->TXT_NRO_CUENTA_BANCARIA . ' POR ' . $cuentabancarias->TXT_NRO_CUENTA_BANCARIA);
			//geolocalización


			return Redirect::to('gestion-de-entrega-documentos/' . $idopcion)->with('bienhecho', 'Se realizo el cambio de cuenta bancaria con exito');
		}

	}

	public function actionCambiarBancoSinCuenta($codbanco, $idoc, $idopcion, Request $request)
	{

		$banco = CMPCategoria::where('COD_CATEGORIA', '=', $codbanco)->first();
		...
		if (count($banco) > 0) {
			...
			FeDocumento::where('ID_DOCUMENTO', $idoc)
				->update(
					[
						'COD_CATEGORIA_BANCO' => $banco->COD_CATEGORIA,
						'TXT_CATEGORIA_BANCO' => $banco->NOM_CATEGORIA,
			...
			$documento->USUARIO_ID = Session::get('usuario')->id;
			$documento->USUARIO_NOMBRE = Session::get('usuario')->nombre;
			$documento->TIPO = 'CAMBIO DE BANCO SIN CUENTA ' . $fe_documento->TXT_CATEGORIA_BANCO . ' POR ' . $banco->NOM_CATEGORIA;
			$documento->MENSAJE = '';
			$documento->save();


			return Redirect::to('gestion-de-entrega-documentos/' . $idopcion)->with('bienhecho', 'Se realizo el cambio de banco con exito');
		}
	...
	public function actionConfigurarDatosGrupoMarketing($orden_id, $idopcion, Request $request)
	{
		...
		$catId = $request['ID_CATCONTAORDEN'];
		$catTxt = '';
		if (!empty($catId)) {
	...
	public function actionBienvenido()
	{
		...
		if ($trol->ind_uc == 1) {
			...
			if (Session::get('usuario')->rol_id == '1CIX00000015' || Session::get('usuario')->rol_id == '1CIX00000016') {
				...
				$listadatos = $this->con_lista_cabecera_comprobante_total_con_liquidacion_compra_anticipo($cod_empresa, $operacion_id);
				$count_x_aprobar_lqa = count($listadatos);
				$lisadatosob = $this->con_lista_cabecera_comprobante_total_con_liquidacion_compra_anticipo_obs($cod_empresa, $operacion_id);
				$count_observados_lqa = count($lisadatosob);
				$listadatosob = $this->con_lista_cabecera_comprobante_total_con_liquidacion_compra_anticipo_obs_levantadas($cod_empresa, $operacion_id);
	...
	public function actionListarPermisos($idopcion)
	{
		...
		$listaroles = WEBRol::where('id', '<>', $this->prefijomaestro . '00000001')->where('ind_merge', '=', 1)->orderBy('nombre', 'asc')->get();
	...
	public function actionAjaxListarOpciones(Request $request)
	{
		...
		$listaopciones = WEBRolOpcion::with(['opcion.grupoopcion'])->where('rol_id', '=', $idrol)->whereIn('rol_id', $array_rols)->get()->sortBy(function($item) { return $item->opcion->nombre; })->groupBy(function($item) { return $item->opcion->grupoopcion->nombre; })->sortBy(function ($val, $key) { return $key; });
	...
	public function actionAjaxModalCambiarClave(Request $request)
	{

		return View::make(
			'usuario/modal/ajax/acambiarclave',
			[
				'ajax' => true,
			]
		);
	...
	public function actionAjaxCambiarClave(Request $request)
	{
		...
		if ($usuario) {
			...
			if ($usuario->passwordmobil != $old_pass) {
				echo "Error: La contraseña antigua no es correcta";
				return;
			}

			if (strlen($password) < 6) {
				echo "Error: La nueva contraseña debe tener al menos 6 caracteres";
				return;
			}

			// Expresion regular para validar fortaleza de contraseña
			if (!preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*(_|[^\w])).+$/', $password)) {
				echo "Error: La contraseña debe contener al menos una mayúscula, una minúscula, un número y un carácter especial";
				return;
			}

			if ($usuario->passwordmobil == $password) {
				echo "Error: La nueva contraseña no puede ser igual a la anterior";
				return;
			}

			$usuario->password = Crypt::encrypt($password);
			$usuario->passwordmobil = $password;
			$usuario->save();
