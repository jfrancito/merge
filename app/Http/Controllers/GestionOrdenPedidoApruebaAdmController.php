...
class GestionOrdenPedidoApruebaAdmController extends Controller
{
    use OrdenPedidoTraits;


    public function actionOrdenPedidoApruebaAdm()
    {
        ...
        return view('ordenpedido.ajax.apruebaordenpedidoadm', [
    ...
    public function insertApruebaOrdenPedidoAdm(Request $request)
    {
        ob_start();
        try {
            ...
            $estado = DB::table('CMP.CATEGORIA')
                ...
                ->where('COD_CATEGORIA', 'ETM0000000000005')
                ->first();
            ...
            ob_clean();
            \Log::error('Error al aprobar orden de pedido adm: ' . $e->getMessage());
            return response()->json(['success' => false, 'mensaje' => $e->getMessage()], 500);
        }
    }

    public function insertRechazarOrdenPedidoAdm(Request $request)
    {
        ob_start();
        try {
            $id_buscar = $request->input('orden_pedido_id');
            ...
            ob_clean();
            \Log::error('Error al rechazar orden de pedido adm: ' . $e->getMessage());
            return response()->json(['success' => false, 'mensaje' => $e->getMessage()], 500);
        }
    }


    public function actionDetallePedidoAdm(Request $request)
    {
        $id_buscar = $request->input('orden_pedido_id');
    ...
    public function actionGuardarEditarDetalleAdm(Request $request)
    {
        $orden_pedido_id = $request->input('orden_pedido_id');
