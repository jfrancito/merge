...
class GestionEntregaDocumentoController extends Controller
{
    ...
    public function actionListarEntregaDocumentoFolio($idopcion)
    {
        ...
        $fechainicio    =   $this->fecha_menos_diez_dias;
        $fechafin       =   $this->fecha_sin_hora;
        ...
        if($rol->ind_uc == 1){
            ...
            if(Session::get('usuario')->id == '1CIX00000086' || Session::get('usuario')->id == '1CIX00000024'){
                ...
                if(Session::get('usuario')->id == '1CIX00000003'){
                    ...
                    $listadatos     =   $listadatos->where('USUARIO_CREA','=',Session::get('usuario')->id);
                }

            }

        }
        $listadatos     =   $listadatos->orderBy('FE_DOCUMENTO_ENTREGABLE.FECHA_CREA','DESC')
                            ->get();
    ...
    public function actionListarAjaxBuscarDocumentoEntregableFolio(Request $request) {
        ...
        $listadatos     =   FeDocumentoEntregable::join('users','users.id','=','FE_DOCUMENTO_ENTREGABLE.USUARIO_CREA')
                            ...
                            ->where(function($query) use ($fecha_inicio, $fecha_fin){
                                if($fecha_inicio != ''){
                                    $query->where('FEC_PAGO','>=',date_format(date_create($fecha_inicio), 'Y-m-d'));
                                }
                                if($fecha_fin != ''){
                                    $query->where('FEC_PAGO','<=',date_format(date_create($fecha_fin), 'Y-m-d'));
                                }
                            });
    ...
    public function actionDescargarDocumentoDICFolio($folio_codigo)
    {
        ...
        Excel::create($titulo, function($excel) use ($listadatossoles,$listadatosdolar,$listadatossolesotro,$listadatosdolarotro,$operacion_id,$funcion,$folio,$empresa) {
            ...
            if($folio->COD_CATEGORIA_MONEDA=='MON0000000000001' or $folio->COD_CATEGORIA_MONEDA==''){
                ...
                $excel->sheet('Soles', function($sheet) use ($listadatossoles,$listadatossolesotro,$operacion_id,$funcion,$folio,$empresa){
                    ...
                    $sheet->loadView('entregadocumento/excel/eentregabledic')->with('listadatos',$listadatossoles)
            ...
            if($folio->COD_CATEGORIA_MONEDA=='MON0000000000002' or $folio->COD_CATEGORIA_MONEDA==''){
                $excel->sheet('Dolares', function($sheet) use ($listadatosdolar,$listadatosdolarotro,$operacion_id,$funcion,$folio,$empresa){
                    ...
                    $sheet->loadView('entregadocumento/excel/eentregabledic')->with('listadatos',$listadatosdolar)
    ...
    public function actionDescargarDocumentoDICConsolidadoFolio($folio_codigo)
    {
        ...
        $listadatossoles    =   $this->con_lista_cabecera_comprobante_entregable_modal_consolidado_moneda($folio->FOLIO,'MON0000000000001');
        $listadatosdolar    =   $this->con_lista_cabecera_comprobante_entregable_modal_consolidado_moneda($folio->FOLIO,'MON0000000000002');
        $listadatossolesotro=   $this->con_lista_cabecera_comprobante_entregable_estiba_modal_moneda_union_consolidado($folio->FOLIO,'MON0000000000001');
        $listadatosdolarotro=   $this->con_lista_cabecera_comprobante_entregable_estiba_modal_moneda_union_consolidado($folio->FOLIO,'MON0000000000002');
        ...
        Excel::create($titulo, function($excel) use ($listadatossoles,$listadatosdolar,$listadatossolesotro,$listadatosdolarotro,$operacion_id,$funcion,$folio,$empresa) {
            ...
            if($folio->COD_CATEGORIA_MONEDA=='MON0000000000001' or $folio->COD_CATEGORIA_MONEDA==''){
                ...
                $excel->sheet('Soles', function($sheet) use ($listadatossoles,$listadatossolesotro,$operacion_id,$funcion,$folio,$empresa){
                    ...
                    $sheet->loadView('entregadocumento/excel/eentregabledicconsolidado')->with('listadatos',$listadatossoles)
                                                                          ->with('listadatosotro',$listadatossolesotro)
                                                                          ->with('funcion',$funcion)
                                                                          ->with('folio',$folio)
                                                                          ->with('empresa',$empresa)
            ...
            if($folio->COD_CATEGORIA_MONEDA=='MON0000000000002' or $folio->COD_CATEGORIA_MONEDA==''){
                $excel->sheet('Dolares', function($sheet) use ($listadatosdolar,$listadatosdolarotro,$operacion_id,$funcion,$folio,$empresa){
                    ...
                    $sheet->loadView('entregadocumento/excel/eentregabledicconsolidado')->with('listadatos',$listadatosdolar)
                                                                          ->with('listadatosotro',$listadatosdolarotro)
                                                                          ->with('funcion',$funcion)
                                                                           ->with('folio',$folio)
                                                                           ->with('empresa',$empresa)
        ...
        Excel::create($titulo, function($excel) use ($lista_bancos,$operacion_id,$folio,$empresa,$funcion) {
            ...
            foreach($lista_bancos as $index => $item){
                ...
                $excel->sheet($npestania, function($sheet) use ($item,$operacion_id,$folio,$empresa,$fedocumento,$listadocumento,$txt_banco,$countfedocu,$funcion){
                    ...
                    $sheet->loadView('entregadocumento/excel/contratopagosmacrocheque')->with('banco',$item)
                                                                               ->with('folio',$folio)
                                                                               ->with('empresa',$empresa)
                                                                               ->with('funcion',$funcion)
                                                                               ->with('txt_banco',$txt_banco)
