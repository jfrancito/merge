...
class GestionOCAdministracionController extends Controller
{
        ...
        if($operacion_id=='ORDEN_COMPRA'){
            ...
            if ($operacion_id == 'ORDEN_COMPRA_ANTICIPO') {
                ...
                if ($operacion_id == 'CONTRATO_ANTICIPO') {
                    $listadatos = $this->con_lista_cabecera_comprobante_contratoa_total_adm($cod_empresa);
                    $listadatos_obs = $this->con_lista_cabecera_comprobante_contratoa_total_adm_obs($cod_empresa);
                    $listadatos_obs_le = $this->con_lista_cabecera_comprobante_contratoa_total_adm_obs_levantadas($cod_empresa);
                } else {
    ...
    public function actionAgregarObservacionAdministracionNotaCredito($idopcion, $linea, $prefijo, $idordencompra,Request $request)
    {
    ...
    public function actionAgregarObservacionAdministracionPG($idopcion, $linea, $prefijo, $idordencompra,Request $request)
    {
    ...
    public function actionAgregarObservacionAdministracionNotaDebito($idopcion, $linea, $prefijo, $idordencompra,Request $request)
    {
    ...
    public function actionAgregarObservacionAdministracionMContratoa($idopcion, $lote, Request $request)
    {
        ...
        if ($_POST) {
            ...
            try {
                ...
                if (count($archivoob) <= 0) {
                    DB::rollback();
                    return Redirect::to('aprobar-comprobante-administracion-contratoa/' . $idopcion . '/' . $lote)->with('errorbd', 'Tiene que seleccionar almenos un item');
                }
    ...
    public function actionAgregarExtornoEstibaAcopio($idopcion, $lote, Request $request)
    {
        ...
        if ($_POST) {
            ...
            try {
                ...
                DB::commit();
                Session::flash('operacion_id', $request['operacion_id']);
                return Redirect::to('/gestion-de-acopio-liquidacion-compra/' . $idopcion)->with('bienhecho', 'Comprobante : ' . $idoc . ' EXTORNADO CON EXITO');
            } catch (\Exception $ex) {
                DB::rollback();
                Session::flash('operacion_id', $request['operacion_id']);
    ...
    public function actionAgregarExtornoAdministracionContratooca($idopcion, $lote, Request $request)
    {
    ...
    public function actionAgregarExtornoAdministracionOC($idopcion, $linea, $prefijo, $idordencompra, Request $request)
    {
        ...
        if ($_POST) {
            ...
            try {
                ...
                DB::commit();
                return Redirect::to('gestion-de-administracion-aprobar/' . $idopcion)->with('bienhecho', 'Comprobante : ' . $ordencompra->COD_ORDEN . ' EXTORNADO CON EXITO');
            } catch (\Exception $ex) {
                DB::rollback();
                return Redirect::to('gestion-de-administracion-aprobar/' . $idopcion)->with('errorbd', $ex . ' Ocurrio un error inesperado');
            }
    ...
    public function actionAprobarAdministracionCONTRATOA($idopcion, $linea, $idordencompra,Request $request)
    {
        ...
        $ordencompra            =   $this->con_lista_cabecera_comprobante_contrato_idoc_actual($idcompra);
        $detalleordencompra     =   $this->con_lista_detalle_contrato_comprobante_idoc($idcompra);
        ...
        if($_POST)
        {
            ...
            try{
                ...
                $documento_top = CMPDocumentoCtble::where('COD_DOCUMENTO_CTBLE', '=', $idcompra)->first();
                ...
                DB::commit();
                Session::flash('operacion_id', 'CONTRATO_ANTICIPO');
                return Redirect::to('/gestion-de-administracion-aprobar/'.$idopcion)->with('bienhecho', 'Comprobante : '.$pedido_id.' APROBADO CON EXITO');
            }catch(\Exception $ex){
                DB::rollback();
                Session::flash('operacion_id', 'CONTRATO_ANTICIPO');
            ...
            //orden de ingreso
            $orden_f                =   CMPDocumentoCtble::where('COD_DOCUMENTO_CTBLE','=',$idcompra)->first();
            $conexionbd         = 'sqlsrv';
            if($orden_f->COD_CENTRO == 'CEN0000000000004'){ //rioja
                $conexionbd         = 'sqlsrv_r';
            }else{
            ...
            $ordencompra_t          =   CMPDocumentoCtble::where('COD_DOCUMENTO_CTBLE','=',$idcompra)->first();
            $codigo_sunat           =   'I';
            $trabajador             =   STDTrabajador::where('NRO_DOCUMENTO','=',$fedocumento->dni_usuariocontacto)->first();
            ...
            $ordencompra_f          =   CMPDocumentoCtble::where('COD_DOCUMENTO_CTBLE','=',$idcompra)->first();
            ...
            $ordencompra          =   CMPDocumentoCtble::where('COD_DOCUMENTO_CTBLE',$idcompra)->first();
            $empresa_sel          =   STDEmpresa::where('COD_EMPR','=',$ordencompra->COD_EMPR_EMISOR)->first();
            $fereftop1            =   FeRefAsoc::where('lote','=',$idoc)->first();
            $funciones = $this;

            //dd($initialPreviewConfig);
            return View::make('comprobante/aprobaradmcontratoa',
                            [
                                'fedocumento'           =>  $fedocumento,
                                'empresa_sel'           =>  $empresa_sel,
                                'fereftop1'             =>  $fereftop1,
    ...
    public function actionAgregarExtornoContratoAdmin($idopcion, $linea, $prefijo, $idordencompra, Request $request)
    {
        ...
        if ($_POST) {
            ...
            try {
                ...
                DB::commit();
                Session::flash('operacion_id', 'CONTRATO');
                return Redirect::to('/gestion-de-administracion-aprobar/' . $idopcion)->with('bienhecho', 'Comprobante : ' . $ordencompra->COD_DOCUMENTO_CTBLE . ' EXTORNADO CON EXITO');
            } catch (\Exception $ex) {
                DB::rollback();
                Session::flash('operacion_id', 'CONTRATO');
    ...
    public function actionAprobarAdministracionNotaCredito($idopcion, $linea,$prefijo, $idordencompra,Request $request)
    {
        ...
        if($_POST)
        {
            ...
            $initialPreview = [];
            foreach ($archivospdf as $archivo) {
                $initialPreview[] = route('serve-filenotacredito', ['file' => $archivo->NOMBRE_ARCHIVO]);
            }
            $initialPreviewConfig = [];
            foreach ($archivospdf as $key => $archivo) {
                $initialPreviewConfig[] = [
                    'type'          => "pdf",
                    'caption' => $archivo->NOMBRE_ARCHIVO,
                    'downloadUrl' => route('serve-filenotacredito', ['file' => $archivo->NOMBRE_ARCHIVO])
                ];
            }
    ...
    public function actionAprobarAdministracionPG($idopcion, $linea,$prefijo, $idordencompra,Request $request)
    {
    ...
    public function actionAprobarAdministracionNotaDebito($idopcion, $linea,$prefijo, $idordencompra,Request $request)
    {
        ...
        if($_POST)
        {
            ...
            $initialPreview = [];
            foreach ($archivospdf as $archivo) {
                $initialPreview[] = route('serve-filenotadebito', ['file' => $archivo->NOMBRE_ARCHIVO]);
            }
            $initialPreviewConfig = [];
            foreach ($archivospdf as $key => $archivo) {
                $initialPreviewConfig[] = [
                    'type'          => "pdf",
                    'caption' => $archivo->NOMBRE_ARCHIVO,
                    'downloadUrl' => route('serve-filenotadebito', ['file' => $archivo->NOMBRE_ARCHIVO])
                ];
            }
    ...
    public function actionAprobarAdministracionLiquidacionCompraAnticipo($idopcion, $linea,$prefijo, $idordenpago,Request $request)
    {
    ...
    public function actionAgregarObservacionAdministracionLiquidacionCompraAnticipo($idopcion, $linea, $prefijo, $idordenpago,Request $request)
    {
