...
class GestionEstibaController extends Controller
{
    ...
    public function actionValidarXMLEstibaAdministratorCONTRATOA($idopcion, $lote,Request $request)
    {
        ...
        if($_POST)
        {
            ...
            try{    
                ...
                if(!is_null($filescdr)){
                    ...
                    foreach($filescdr as $file){
                        ...
                        if ($zip->open($file->getPathname()) === TRUE) {
                            ...
                            DB::rollback(); 
                            return Redirect::to('detalle-comprobante-estiba-administrator-contrato-anticipo/'.$idopcion.'/'.$lote)->with('errorbd', 'No se pudo abrir el archivo .zip');
                        }
                ...
                DB::rollback();
                return Redirect::to('detalle-comprobante-estiba-administrator-contrato-anticipo/'.$idopcion.'/'.$lote)->with('errorbd', $ex.' Ocurrio un error inesperado');
            }
    ...
    public function actionValidarXMLEstibaAdministratorOCA($idopcion, $lote,Request $request)
    {
        ...
        if($_POST)
        {
            ...
            try{    
                ...
                $documento_asociados    =   CMPOrden::whereIn('COD_ORDEN',$lotes)->get();
                ...
                DB::rollback();
                return Redirect::to('detalle-comprobante-estiba-administrator-compra-anticipo/'.$idopcion.'/'.$lote)->with('errorbd', $ex.' Ocurrio un error inesperado');
            }
            return Redirect::to('gestion-de-orden-compra/'.$idopcion)->with('bienhecho', 'Se valido el xml correctamente');
    ...
    public function actionDetalleSelectEstibaOrdenCompraAnticipo($idopcion,Request $request)
    {
        $jsondocumenos                          =    json_decode($request['jsondocumenos'],true);
        $operacion_id                           =    $request['operacion_sel'];
        $lote                                   =    $this->funciones->generar_lote('FE_REF_ASOC',8);
        ...
        return Redirect::to('detalle-comprobante-estiba-administrator-compra-anticipo/'.$idopcion.'/'.$lote);
    }



    public function actionDetalleSelectEstibaContratoAnticipo($idopcion,Request $request)
    {
        $jsondocumenos                          =    json_decode($request['jsondocumenos'],true);
        $operacion_id                           =    $request['operacion_sel'];
        $lote                                   =    $this->funciones->generar_lote('FE_REF_ASOC',8);
        ...
        return Redirect::to('detalle-comprobante-estiba-administrator-contrato-anticipo/'.$idopcion.'/'.$lote);
    }
    ...
    public function actionDetalleComprobanteestibaAdministratorCompraAnticipo($idopcion, $lote, Request $request) {
        ...
        if(count($fedocumento)>0){
            ...
            if($fedocumento->OPERACION_DET == 'SIN_XML'){
                $tarchivos              =   CMPDocAsociarCompra::where('COD_ORDEN','=',$lote)->where('COD_ESTADO','=',1)
                                            ->get();   
            }
        ...
        $combodocumento         =   array('DCC0000000000002' => 'FACTURA ELECTRONICA');
        ...
        $COD_CENTRO             =   '';
        $FEC_CORTE              =   $this->hoy_sh;
        $CLIENTE                =   $documento_top->TXT_EMPR_CLIENTE;
        $COD_MONEDA             =   $documento_top->COD_CATEGORIA_MONEDA;
        $monto_anticipo         =   0.00;
        $stmt = DB::connection('sqlsrv')->getPdo()->prepare('SET NOCOUNT ON;EXEC CMP.OBTENER_ADELANTOS_PROVEEDOR_DETALLADO 
                                                                @COD_EMPR = ?,
        ...
        $detalleordencompra     =   CMPDetalleProducto::where('CMP.DETALLE_PRODUCTO.COD_ESTADO','=',1)
                                                        ->whereIn('CMP.DETALLE_PRODUCTO.COD_TABLA',$lotes)
                                                        ->orderBy('NRO_LINEA','ASC')
                                                        ->get();

        return View::make('comprobante/registrocomprobanteestibaadministratoroca',
                         [
                            'monto_anticipo'        =>  $monto_anticipo,
                            'comboant'              =>  $comboant,
                            'combotipodetraccion'   =>  $combotipodetraccion,
    ...
    public function actionDetalleComprobanteestibaAdministratorContratoAnticipo($idopcion, $lote, Request $request) {
        ...
        if(count($fedocumento)>0){
            ...
            $ordencompra              =   CMPDocumentoCtble::whereIn('COD_DOCUMENTO_CTBLE',$lotes)->first();
        ...
        $COD_CENTRO             =   '';
        $FEC_CORTE              =   $this->hoy_sh;
        $CLIENTE                =   $documento_top->TXT_EMPR_EMISOR;
        $COD_MONEDA             =   $documento_top->COD_CATEGORIA_MONEDA;
        $monto_anticipo         =   0.00;
        ...
        //dd($ordencompra->COD_DOCUMENTO_CTBLE);
        if($ordencompra_f->COD_CENTRO == 'CEN0000000000004' or $ordencompra_f->COD_CENTRO == 'CEN0000000000006'){
            if($ordencompra_f->COD_CENTRO == 'CEN0000000000004'){
        ...
        return View::make('comprobante/registrocomprobanteestibaadministratorcont',
                         [
                            'monto_anticipo'        =>  $monto_anticipo,
                            'comboant'              =>  $comboant,
                            'combotipodetraccion'   =>  $combotipodetraccion,
                        ...
                        if($documento_id=='DCC0000000000002'){
                            ...
                            if($request['operacion_id'] == 'DOCUMENTO_INTERNO_COMPRA'){
                                ...
                                $archivosdelfe      =   CMPCategoria::where('TXT_GRUPO','=','DOCUMENTOS_COMPRA')
                                                        ->whereIn('COD_CATEGORIA', ['DCC0000000000002','DCC0000000000003','DCC0000000000004','DCC0000000000007','DCC0000000000008','DCC0000000000001'])
                                                        ->get();
    ...
    public function actionCargarXMLEstibaAdministratorOrdenCompraAnticipo($idopcion, $lote,Request $request)
    {
        ...
        if($_POST)
        {
            ...
            try{    
                    ...
                    if($tipodocumento_id=='SIN_CPE'){
                        ...
                        $documento->TXT_PROCEDENCIA         =   'ADM';
                        $documento->ESTADO                  =   'A';
                        $documento->RUC_PROVEEDOR           =   $empresa_sel->NRO_DOCUMENTO;
                        ...
                        $documento->SUB_TOTAL_VENTA_SOLES   =   $fereftop1->TOTAL_MERGE;
                        $documento->TOTAL_VENTA_ORIG        =   $fereftop1->TOTAL_MERGE;
                        $documento->TOTAL_VENTA_SOLES       =   $fereftop1->TOTAL_MERGE;
                        $documento->PERCEPCION              =   0;
                        $documento->MONTO_RETENCION         =   0;
                        $documento->HORA_EMISION            =   date_format(date_create($ordencompra->FEC_ORDEN), 'h:i:s');
                        ...
                        $documento->MONTO_NC                =   0.00;
                        $documento->MONTO_DETRACCION_XML    =   (float)$monto_detraccion;
                        $documento->MONTO_DETRACCION_RED    =   round((float)$monto_detraccion);
                        $documento->MONTO_RETENCION         =   $cant_rentencion;
                        $documento->save();
                        ...
                        if (!empty($file)) 
                        {
                            ...
                            $tp                   =   CMPCategoria::where('COD_CATEGORIA','=',$ordencompra->COD_CATEGORIA_TIPO_PAGO)->first();
                            $empresa_sel          =   STDEmpresa::where('COD_EMPR','=',$ordencompra->COD_EMPR_CLIENTE)->first();
                            $fereftop1              =   FeRefAsoc::where('lote','=',$fedocumento->ID_DOCUMENTO)->first();

                            $this->con_validar_documento_anticipo($ordencompra,$fedocumento,$detalleordencompra,$detallefedocumento,$empresa_sel,$fereftop1);
                            ...
                            return Redirect::to('detalle-comprobante-estiba-administrator-compra-anticipo/'.$idopcion.'/'.$idoc)->with('errorurl', 'Seleccione Archivo XML a Importar ');
                        }
                    }
                ...
                DB::rollback(); 
                return Redirect::to('detalle-comprobante-estiba-administrator-compra-anticipo/'.$idopcion.'/'.$idoc)->with('errorbd', $ex.' Ocurrio un error inesperado');
            }
            return Redirect::to('detalle-comprobante-estiba-administrator-compra-anticipo/'.$idopcion.'/'.$idoc)->with('bienhecho', 'Se valido el xml');


        }
    }

    public function actionCargarXMLEstibaAdministratorContratoAnticipo($idopcion, $lote,Request $request)
    {
        ...
        if($_POST)
        {
            ...
            try{    
                    ...
                    if($tipodocumento_id=='SIN_CPE'){
                        ...
                        $documento->TIPO_CLIENTE            =   '';
                        $documento->ID_CLIENTE              =   $empresa_sel->NRO_DOCUMENTO;
                        $documento->NOMBRE_CLIENTE          =   $ordencompra->TXT_EMPR_EMISOR;
                        $documento->DIRECCION_CLIENTE       =   $ordencompra->COD_CENTRO;
                        $documento->SERIE                   =   $letras;
                        $documento->NUMERO                  =   $numeros_sin_ceros;
                        $documento->ID_TIPO_DOC             =   '';
                        ...
                        if (!empty($file)) 
                        {
                            ...
                            $fedocumento_e          =   FeDocumento::whereNotIn('COD_ESTADO',['','ETM0000000000006'])
                                                        ...
                                                        ->first();
                            if(count($fedocumento_e)>0){
                                return Redirect::back()->with('errorurl', 'Este XML ya fue integrado en otro contrato');
                            }
                            ...
                            $this->con_validar_documento_anticipo_contrato($ordencompra,$fedocumento,$detalleordencompra,$detallefedocumento,$empresa_sel,$fereftop1);
                            ...
                            //FRANK
                            $archivosdelfe      =   CMPCategoria::where('TXT_GRUPO','=','DOCUMENTOS_COMPRA')
                                                    ->whereIn('COD_CATEGORIA', ['DCC0000000000026','DCC0000000000002','DCC0000000000003','DCC0000000000004','DCC0000000000012','DCC0000000000009'])->get();
                            ...
                            return Redirect::to('detalle-comprobante-estiba-administrator-contrato-anticipo/'.$idopcion.'/'.$idoc)->with('errorurl', 'Seleccione Archivo XML a Importar ');
                        }
                    }
                ...
                DB::rollback(); 
                return Redirect::to('detalle-comprobante-estiba-administrator-contrato-anticipo/'.$idopcion.'/'.$idoc)->with('errorbd', $ex.' Ocurrio un error inesperado');
            }
            return Redirect::to('detalle-comprobante-estiba-administrator-contrato-anticipo/'.$idopcion.'/'.$idoc)->with('bienhecho', 'Se valido el xml');
