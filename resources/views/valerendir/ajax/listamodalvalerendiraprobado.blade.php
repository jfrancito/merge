<table class="table table-striped table-borderless" style="font-style: italic;">
    ...
    @foreach($listarusuarios as $index=>$item)
     @if(($item['TXT_CATEGORIA_ESTADO_VALE'] === 'AUTORIZADO' || $item['TXT_CATEGORIA_ESTADO_VALE'] === 'APROBADO' || $item['TXT_CATEGORIA_ESTADO_VALE'] === 'RECHAZADO' ) && $item['USUARIO_APRUEBA_ID'] === $usuario_logueado_id)
        <tr data_vale_rendir="{{$item['ID']}}">
         ...
         <td class="custom-glosa1">
                <div class="dropdown">
                   <button class="btn btn-sm btn-outline-dark dropdown-toggle text-left btn-primary"
                 ...
                 <div class="dropdown-menu d-dropdown-menu shadow-sm p-1" aria-labelledby="dropdownAcciones{{ $item['ID'] }}">
                        ...
                        @if(!empty($item['TXT_GLOSA_APROBADO']))
