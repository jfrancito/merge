            ...
            <tbody class="no-border-x">
              @foreach($cuentabancarias as $index => $item)
                  <tr>
                    ...
                    <td>
                      <a href="{{ url('/cambiar-cuenta-corriente-impulso/'.$item->COD_EMPR_TITULAR.'/'.$item->COD_EMPR_BANCO.'/'.$item->TXT_NRO_CUENTA_BANCARIA.'/'.$item->COD_CATEGORIA_MONEDA.'/'.$idoc.'/'.$idopcion) }}" class="tools select"> <span class="label label-success">SELECCINAR CUENTA</span></a>
                    </td>
                  </tr>
              @endforeach
            </tbody>
