    ...
    @foreach($planillamovilidad as $index=>$item)
      <tr>
        ...
        <td class="cell-detail" style="position: relative;">
          ...
          <a href="{{ url('/modificar-movilidad-venta-masivo/'.$idopcion.'/'.Hashids::encode(substr($item->ID_DOCUMENTO, -8))) }}" 
          ...
          @if($item->COD_ESTADO != 'ETM0000000000001')
            <a href="{{ url('/seguimiento-movilidad-venta/'.$idopcion.'/'.Hashids::encode(substr($item->ID_DOCUMENTO, -8))) }}" style="margin-top: 5px;float: right;" class="btn btn-rounded btn-space btn-primary btn-sm">SEGUIMIENTO</a>
          @endif
          @if($item->COD_ESTADO == 'ETM0000000000001')
