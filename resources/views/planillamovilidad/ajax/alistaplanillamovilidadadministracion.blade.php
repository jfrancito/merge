      ...
      <div class="panel panel-default">
        <div class="tab-container">
          <ul class="nav nav-tabs">
            <li class="@if($tab_id=='oc') active @endif"><a href="#oc" data-toggle="tab">PLANILLA MOVILIDAD <span class="badge badge-success" style="font-size:16px">{{count($listadatos)}}</span></a></li>
          ...
          <div class="tab-content">
            <div id="oc" class="tab-pane @if($tab_id=='oc') active @endif cont">
              @include('planillamovilidad.lista.ajax.alistapmadmin', ['id' => 'nso' , 'listadatos' => $listadatos])
            </div>
            <div id="observado" class="tab-pane @if($tab_id=='observado') active @endif cont">
              @include('planillamovilidad.lista.ajax.alistapmadmin', ['id' => 'nso_obs' , 'listadatos' => $listadatos_obs])
            </div>
            <div id="observadole" class="tab-pane @if($tab_id=='observadole') active @endif cont">
              @include('planillamovilidad.lista.ajax.alistapmadmin', ['id' => 'nso_obs_le' , 'listadatos' => $listadatos_obs_le])
            </div>
          </div>
        </div>
      </div>
