@if(count($productos) > 0)
    <div class="row" style="margin-bottom: 15px; padding: 0 5px;">
        <div class="col-md-12">
            <p class="text-muted">
                Seleccione los productos del consolidado <b>{{ $id_consolidado }}</b> que desea deshabilitar. Solo se muestran los productos que aún no han sido cotizados.
            </p>
        </div>
    </div>
            ...
            <tbody>
                ...
                <tr class="fila-producto-deshabilitar" style="cursor: pointer;">
                    <td class="text-center">
                        <div class="xs-check-prod">
                            <input id="check-prod-{{ trim($prod->COD_PRODUCTO) }}" 
                                   type="checkbox" 
                                   name="cod_producto_desh[]" 
                                   class="check-producto-desh" 
                                   value="{{ trim($prod->COD_PRODUCTO) }}"
                                   checked>
                    ...
                    <td class="text-right font-bold">{{ number_format($prod->CANTIDAD, 2) }}</td>
...
@else
    <div class="alert alert-warning text-center shadow-soft" style="margin: 20px 0; border-radius: 8px;">
        ...
        <p>Todos los productos de este consolidado (<b>{{ $id_consolidado }}</b>) ya están asociados a una cotización activa.</p>
    </div>
    <script>
        $(document).ready(function() {
