@if(count($productos) > 0)
    <div class="row" style="margin-bottom: 15px; padding: 0 5px;">
        <div class="col-md-12">
            <p class="text-muted">
                Seleccione los productos del consolidado <b>{{ $id_consolidado }}</b> que desea deshabilitar. Solo se
                muestran los productos que aún tienen saldo pendiente de cotizar.
            </p>
        </div>
    </div>
        ...
        <table id="table-productos-deshabilitar" class="table table-striped table-hover table-fw-widget listatabla">
            <thead>
                <tr>
                    ...
                    <th>PRODUCTO</th>
                    <th>UNIDAD MEDIDA</th>
                    <th class="text-right">CANT. CONSOLIDADA</th>
                ...
                @foreach($productos as $prod)
                    <tr class="fila-producto-deshabilitar" style="cursor: pointer;">
                        <td class="text-center">
                            <div class="xs-check-prod">
                                <input id="check-prod-{{ trim($prod->COD_PRODUCTO) }}" type="checkbox"
                                    name="cod_producto_desh[]" class="check-producto-desh"
                                    value="{{ trim($prod->COD_PRODUCTO) }}" checked>
                                <label for="check-prod-{{ trim($prod->COD_PRODUCTO) }}"></label>
                            </div>
                        </td>
                        <td class="font-bold">{{ trim($prod->COD_PRODUCTO) }}</td>
                        <td>{{ $prod->NOM_PRODUCTO }}</td>
                        <td>{{ $prod->NOM_CATEGORIA_MEDIDA }}</td>
                        <td class="text-center">{{ number_format($prod->CANTIDAD, 2) }}</td>
                        <td class="text-center font-bold text-danger">{{ number_format($prod->SALDO_PENDIENTE, 2) }}</td>
                        <td>{{ $prod->NOM_CATEGORIA_FAMILIA }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
    ...
    <input type="hidden" id="id_consolidado_desh" value="{{ $id_consolidado }}">
...
@else
    <div class="alert alert-warning text-center shadow-soft" style="margin: 20px 0; border-radius: 8px;">
        ...
        <p>Todos los productos de este consolidado (<b>{{ $id_consolidado }}</b>) ya están asociados a una cotización
            activa.</p>
    </div>
    <script>
        $(document).ready(function () {
...
<style>
    ...
    .xs-check-prod input:checked+label {
        background: #a94442;
        border-color: #a94442;
    }
