                ...
                <div class="text-center">
                    ...
                    <div class="d-flex justify-content-center align-items-center gap-3">
                        ...
                        <span style="font-size: 16px; color: rgba(255,255,255,0.85); font-weight: 500;">
                            ...
                            <b>{{ date('d-m-Y', strtotime($pedido->FEC_PEDIDO)) }}</b>
                        </span>
                    </div>
                    <div class="mt-1">
            ...
            <div class="panel-body" style="padding: 30px 40px;">
                ...
                <div class="row mb-5"
                    ...
                    <div class="col-md-4">
                        ...
                        <p class="mb-0 text-dark fw-bold" style="font-size: 15px;">
                            {{ $pedido->TXT_TRABAJADOR_SOLICITA }}</p>
                    </div>
                    <div class="col-md-4">
                        <label class="text-muted small fw-bold text-uppercase mb-1 d-block"
                            ...
                            @foreach($pedillodetalle as $index => $detalle)
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <span
                                            class="badge-corpo {{ $detalle->IND_MATERIAL_SERVICIO == 'M' ? 'bg-light text-primary' : 'bg-light text-warning' }}">
                                    ...
                                    <td class="text-center">
                                        @if ($pedido->COD_TRABAJADOR_AUTORIZA == $cod_usuario_session && $pedido->COD_ESTADO == 'ETM0000000000010')
                                            <input type="number" class="form-control text-center input-cantidad-editar input-sm"
                                                value="{{ (int) $cantidad_mostrar }}" min="1" data-id="{{ $detalle->ID_PEDIDO }}"
                                                data-prod="{{ $detalle->COD_PRODUCTO }}"
                                                style="width: 80px; margin: 0 auto; font-weight: bold; border-color: #d1d3e2;">
                                        @else
                                            <span class="fw-bold text-dark"
                                                style="font-size: 15px;">{{ (int) $cantidad_mostrar }}</span>
                                        @endif
                                    </td>
                                    <td class="text-center">
                                    ...
                                    <td class="text-center fw-bold">S/ {{ number_format($precio, 2) }}</td>
                                    <td class="text-center fw-bold text-dark">S/ {{ number_format($subtotal, 2) }}</td>
                                </tr>
                            @endforeach
                        </tbody>
                        <tfoot>
                ...
                @if(isset($pedido->TXT_GLOSA_RECHAZO) && !empty($pedido->TXT_GLOSA_RECHAZO))
                    <div class="alert alert-danger mt-4"
                        ...
                        <p class="mb-0" style="font-size: 14px; color: #000 !important;">{{ $pedido->TXT_GLOSA_RECHAZO }}
                        </p>
                    </div>
                @endif
                ...
                <div class="d-flex justify-content-end align-items-center gap-3 mt-5 pt-4"
                    ...
                    @if ($pedido->COD_TRABAJADOR_AUTORIZA == $cod_usuario_session && $pedido->COD_ESTADO == 'ETM0000000000010')
                        ...
                        <button class="btn-corpo btn-corpo-success autorizar-pedido" data-id="{{ $pedido->ID_PEDIDO }}">
                            <i class="fa fa-check-circle me-1"></i> Autorizar Pedido
                        </button>

                        <button class="btn-corpo btn-corpo-danger rechazar-pedido" data-id="{{ $pedido->ID_PEDIDO }}">
