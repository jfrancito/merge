  ...
  <div class="col-sm-6">
    ...
    @foreach($documentoscompra as $index => $item)
      <div class="be-checkbox">
        <input id="{{$item->COD_CATEGORIA}}" value="{{$item->COD_CATEGORIA}}"  type="checkbox" name="archivoob[]"  @if(in_array($item->COD_CATEGORIA, $totalarchivos)) checked  disabled @endif >
        <label for="{{$item->COD_CATEGORIA}}">{{$item->NOM_CATEGORIA}} ({{$item->COD_CTBLE}})</label>
      </div>
    @endforeach
