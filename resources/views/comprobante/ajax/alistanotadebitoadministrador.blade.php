            ...
            <ul role="menu" class="dropdown-menu pull-right">
              <li>
                @if(ltrim(rtrim($item->COD_ESTADO)) == '') 
                    <a href="{{ url('/detalle-comprobante-nota-debito-administrator/'.$procedencia.'/'.$idopcion.'/'.substr($item->COD_DOCUMENTO_CTBLE, 0,6).'/'.Hashids::encode(substr($item->COD_DOCUMENTO_CTBLE, -10))) }}">
                      Registro XML
                    </a>
                @else
                  ...
                  @else
                    @if($item->COD_ESTADO != 'ETM0000000000001')
                      @if($item->COD_ESTADO != 'ETM0000000000006')
                        <a href="{{ url('/detalle-comprobante-nota-debito-validado/'.$idopcion.'/'.substr($item->COD_DOCUMENTO_CTBLE, 0,6).'/'.Hashids::encode(substr($item->COD_DOCUMENTO_CTBLE, -10))) }}">
                          Detalle de Registro
                        </a>
                      @else
                        <a href="{{ url('/detalle-comprobante-nota-debito-administrator/'.$procedencia.'/'.$idopcion.'/'.substr($item->COD_DOCUMENTO_CTBLE, 0,6).'/'.Hashids::encode(substr($item->COD_DOCUMENTO_CTBLE, -10))) }}">
