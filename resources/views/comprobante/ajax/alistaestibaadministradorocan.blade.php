...
<div class="panel-heading">
  <form method="POST" id='formre' action="{{ url('/select-xml-estiba-orden-compra-anticipo/'.$idopcion) }}" style="border-radius: 0px;" class="form-horizontal group-border-dashed" enctype="multipart/form-data">
        {{ csrf_field()}}
...
<table id="estiba" class="table table-striped table-borderless table-hover td-color-borde td-padding-7 listatabla">
  ...
  <tbody>
    @foreach($listadatos as $index => $item)
      <tr data_requerimiento_id = "{{$item->COD_ORDEN}}" data_lote = "{{$item->LOTE_DOC}}" data_total = "{{$item->CAN_TOTAL}}" data_mergetotal = "{{$item->TOTAL_MERGE}}">
        <td><b>{{$index + 1}}</b></td>
